...
export default function DatasetListPage() {
    ...
    const handleDatasetActivate = (dataset: Dataset) => {
        navigate(`/datasets/${dataset.id}`, {state: {dataset}})
    }

    const handleDatasetRowKeyDown = (event: KeyboardEvent<HTMLTableRowElement>, dataset: Dataset) => {
        if (event.key === "Enter" || event.key === " ") {
            event.preventDefault()
            handleDatasetActivate(dataset)
        }
    }
    ...
    return (
        <main className="min-h-screen bg-gradient-to-b from-background via-muted/40 to-background">
            <section className="mx-auto flex w-full max-w-5xl flex-col gap-8 px-4 pb-16 pt-12 sm:px-6 lg:px-12">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Button asChild variant="outline" className="gap-2">
                            <Link to="/create">
                                <ArrowLeft className="h-4 w-4"/>ایجاد دیتاست جدید
                            </Link>
                        </Button>
                    </div>
                </div>
                ...
                <Card
                    className="border-border/70 shadow-lg shadow-primary/5 supports-[backdrop-filter]:backdrop-blur-sm">
                                ...
                                <TableBody>
                                    {formattedDatasets.map((dataset) => (
                                        <TableRow
                                            ...
                                            role="button"
                                            tabIndex={0}
                                            onClick={() => handleDatasetActivate(dataset)}
                                            onKeyDown={(event) => handleDatasetRowKeyDown(event, dataset)}
                                            className="cursor-pointer transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 hover:bg-muted/60"
                                        >
                                            ...
                                            <TableCell
                                                className="text-right text-muted-foreground">{dataset.importedAtLabel}</TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                            </Table>
